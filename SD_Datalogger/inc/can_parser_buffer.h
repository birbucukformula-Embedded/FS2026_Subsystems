 ...
 */
#define CAN_CSV_HEADER_STRING       "Timestamp(ms),Speed(km/h),Temp(C),ErrorCode\r\n"

/**
 * @brief  Sanal CAN Bus Veri Paketi Tanımı
 * @details 2. Kişinin simüle edip CSV formatına dönüştüreceği temel telemetri veri yapısı.
 */
typedef struct {
    uint32_t timestamp_ms;  /**< Milisaniye cinsinden zaman damgası */
    float    speed_kmh;     /**< Araç hızı (km/h) */
    float    engine_temp_c; /**< Motor / İnverter sıcaklığı (°C) */
    uint8_t  error_code;    /**< Alt sistem hata kodu (0 = Normal, >0 = Hata) */
} CAN_DataFrame_t;
 ...
 * 
 * @param[out] frame Üretilen sanal verinin yazılacağı CAN_DataFrame_t adresi.
 */
 ...
 * @details Örnek Çıktı: "1050,85.50,90.20,0\r\n"
 * 
 * @param[in]  frame    Formatlanacak ham telemetri verisi.
 ...
 * 
 * @param[in] frame Tampona eklenecek telemetri verisi.
